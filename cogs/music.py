...
class Music(commands.Cog):
    ...
    async def queue_info(self, ctx):
        ...
        fmt = '\n'.join(f"`{(upcoming.index(song)) + 1}.` [{song['title']}]({song['webpage_url']}) | ` {duration} Requested by: {song['requester']}`\n" for song in upcoming)
        fmt = f"\n__Now Playing__:\n[{vc.source.title}]({vc.source.web_url}) | ` {duration} Requested by: {str(vc.source.requester).split('#')[0]}`\n\n__Up Next:__\n" + fmt + f"\n**{len(upcoming)} songs in queue**"
